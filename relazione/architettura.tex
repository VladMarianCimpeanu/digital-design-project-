...
La macchina alza il segnale init che ha lo scopo di inizializzare correttamente i registri max register e min register: in caso di letture consecutive di immagini, il reset della macchina avverrebbe una volta sola, mentre max register e min register andrebbero resettati all'inizio della lettura di ciascuna immagine\footnote{Nel caso in cui il massimo e il minimo dell'immagine precedente fossero rispettivamente maggiore e minore del massimo e del minimo dell'immagine successiva, verrebbero mantenuti i valori precedenti di massimo e minimo.}.
...
Infine, viene caricato il valore del registro Address in end register, che tornerà utile in seguito.
...
\begin{itemize}
    \item Address: contiene l'indirizzo in cui verrà scritto l'immagine equalizzata. %il nuovo pixel
    \item Address2: contiene l'indirizzo da cui verrà letta l'immagine da equalizzare. %il pixel originale
\end{itemize}
